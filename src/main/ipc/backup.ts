import { ipcMain, dialog } from 'electron'
import { validateIpc, validateIpcAsync, validateNoArgs, idSchema } from '../lib/ipc-validate'
import { zipPathSchema, workspaceNameSchema, workspacePathSchema } from './schemas'
import { backupService } from '../services/backup'
import { workspaceService } from '../services/workspace'

export function registerBackupHandlers(): void {
  ipcMain.handle(
    'backup:export',
    validateIpcAsync([idSchema], async (workspaceId) => {
      const ws = workspaceService.getById(workspaceId)
      const timestamp = new Date().toISOString().replace(/[:.]/g, '-').slice(0, 19)
      const { canceled, filePath } = await dialog.showSaveDialog({
        defaultPath: `rally-backup-${ws.name}-${timestamp}.zip`,
        filters: [{ name: 'ZIP', extensions: ['zip'] }]
      })
      if (canceled || !filePath) return null
      return backupService.export(workspaceId, filePath)
    })
  )

  ipcMain.handle(
    'backup:selectFile',
    validateNoArgs(async (): Promise<string | null> => {
      const { canceled, filePaths } = await dialog.showOpenDialog({
        filters: [{ name: 'Rally Backup', extensions: ['zip'] }],
        properties: ['openFile']
      })
      return canceled ? null : filePaths[0]
    })
  )

  // 보안-1 Phase 3: zipPath path traversal 차단 + .zip 확장자 강제.
  ipcMain.handle(
    'backup:readManifest',
    validateIpc([zipPathSchema], (zipPath) => backupService.readManifest(zipPath))
  )

  // 보안-1 Phase 3: zipPath / name / 새 워크스페이스 path 검증.
  ipcMain.handle(
    'backup:import',
    validateIpcAsync(
      [zipPathSchema, workspaceNameSchema, workspacePathSchema] as const,
      (zipPath, name, path) => backupService.import(zipPath, name, path)
    )
  )
}
