...
describe('findAllDescendantIds', () => {
  ...
  it('1단계 자식 → 자식 ID 배열', () => {
    testDb
      .insert(schema.todos)
      .values(makeTodo({ id: 'parent' }))
      .run()
    testDb
      .insert(schema.todos)
      .values(makeTodo({ id: 'child-1', parentId: 'parent' }))
      .run()
    testDb
      .insert(schema.todos)
      .values(makeTodo({ id: 'child-2', parentId: 'parent' }))
      .run()
  ...
  it('다단계 (parent→child→grandchild) → 모든 하위 ID', () => {
    testDb
      .insert(schema.todos)
      .values(makeTodo({ id: 'p' }))
      .run()
    testDb
      .insert(schema.todos)
      .values(makeTodo({ id: 'c', parentId: 'p' }))
      .run()
    testDb
      .insert(schema.todos)
      .values(makeTodo({ id: 'gc', parentId: 'c' }))
      .run()
