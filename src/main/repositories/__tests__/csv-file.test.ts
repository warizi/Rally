...
describe('findByWorkspaceId', () => {
  ...
  it('해당 workspace의 csv만 반환 (다른 ws 제외)', () => {
    testDb
      .insert(schema.workspaces)
      .values({ id: 'ws-2', name: 'Other', path: '/other', createdAt: new Date(), updatedAt: new Date() })
      .run()
    testDb.insert(schema.csvFiles).values(makeCsv({ id: 'c1' })).run()
    testDb.insert(schema.csvFiles).values(makeCsv({ id: 'c2', workspaceId: 'ws-2', relativePath: 'other.csv' })).run()
...
describe('findById', () => {
  it('존재하는 id → CsvFile 반환', () => {
    testDb.insert(schema.csvFiles).values(makeCsv({ id: 'x1' })).run()
...
describe('findByRelativePath', () => {
  it('workspaceId + relativePath 일치 → CsvFile 반환', () => {
    testDb.insert(schema.csvFiles).values(makeCsv({ id: 'rp1', relativePath: 'data/a.csv' })).run()
...
describe('createMany', () => {
  ...
  it('중복 relativePath → onConflictDoNothing (에러 없음)', () => {
    testDb.insert(schema.csvFiles).values(makeCsv({ id: 'dup1', relativePath: 'dup.csv' })).run()
    expect(() =>
...
describe('update', () => {
  it('지정 필드만 변경, 나머지 보존', () => {
    testDb.insert(schema.csvFiles).values(makeCsv({ id: 'u1', title: '원본', description: '설명' })).run()
...
describe('deleteOrphans', () => {
  it('existingPaths에 없는 csv 삭제', () => {
    testDb.insert(schema.csvFiles).values(makeCsv({ id: 'o1', relativePath: 'keep.csv' })).run()
    testDb.insert(schema.csvFiles).values(makeCsv({ id: 'o2', relativePath: 'orphan.csv' })).run()
  ...
  it('existingPaths 빈 배열 → 전체 삭제', () => {
    testDb.insert(schema.csvFiles).values(makeCsv({ id: 'o3', relativePath: 'a.csv' })).run()
    testDb.insert(schema.csvFiles).values(makeCsv({ id: 'o4', relativePath: 'b.csv' })).run()
  ...
  it('모두 existingPaths에 있으면 삭제 없음', () => {
    testDb.insert(schema.csvFiles).values(makeCsv({ id: 'o5', relativePath: 'x.csv' })).run()
    testDb.insert(schema.csvFiles).values(makeCsv({ id: 'o6', relativePath: 'y.csv' })).run()
...
describe('bulkDeleteByPrefix', () => {
  it('prefix 하위 csv만 삭제, 나머지 보존', () => {
    testDb.insert(schema.csvFiles).values(makeCsv({ id: 'bp1', relativePath: 'docs/a.csv' })).run()
    testDb.insert(schema.csvFiles).values(makeCsv({ id: 'bp2', relativePath: 'docs/sub/b.csv' })).run()
    testDb.insert(schema.csvFiles).values(makeCsv({ id: 'bp3', relativePath: 'other.csv' })).run()
...
describe('bulkUpdatePathPrefix', () => {
  it('정확히 oldPrefix와 일치하는 경로 → newPrefix로 변경', () => {
    testDb.insert(schema.csvFiles).values(makeCsv({ id: 'pp1', relativePath: 'old-folder' })).run()
  ...
  it('oldPrefix/ 하위 경로 → newPrefix/ 하위로 변경', () => {
    testDb.insert(schema.csvFiles).values(makeCsv({ id: 'pp2', relativePath: 'old-folder/a.csv' })).run()
    testDb.insert(schema.csvFiles).values(makeCsv({ id: 'pp3', relativePath: 'other/b.csv' })).run()
    csvFileRepository.bulkUpdatePathPrefix(WS_ID, 'old-folder', 'new-folder')
  ...
  it('updatedAt 갱신 확인', () => {
    ...
    testDb.insert(schema.csvFiles).values(makeCsv({ id: 'pp4', relativePath: 'old/c.csv', updatedAt: oldDate })).run()
...
describe('reindexSiblings', () => {
  it('orderedIds 순서대로 order 재설정', () => {
    testDb.insert(schema.csvFiles).values(makeCsv({ id: 'ri1', relativePath: 'r1.csv', order: 5 })).run()
    testDb.insert(schema.csvFiles).values(makeCsv({ id: 'ri2', relativePath: 'r2.csv', order: 3 })).run()
...
describe('delete', () => {
  it('삭제 후 findById → undefined', () => {
    testDb.insert(schema.csvFiles).values(makeCsv({ id: 'd1' })).run()
