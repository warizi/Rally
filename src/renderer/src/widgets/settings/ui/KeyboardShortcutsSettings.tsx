 ...
 * 표기는 OS별 분기 — mac: ⌘/⌥/⇧, Windows/Linux: Ctrl/Alt/Shift.
 */
...
// platform 판별이 런타임 값이므로 모듈 상수 대신 렌더 시점에 생성
function buildGroups(): ShortcutGroup[] {
  ...
  // 설명 문장용 소문자 표기 (mac: cmd/opt, win: ctrl/alt)
  const primaryText = isMac() ? 'cmd' : 'ctrl'
  const altText = isMac() ? 'opt' : 'alt'
  ...
  return [
    {
      ...
      rows: [
        {
          label: '전체 검색',
          keys: <Combo keys={[primary, shift]} trigger="F" />,
          description: `${primaryText} + shift + f 를 누르면 전체 검색 다이얼로그가 열립니다. 노트 · 표 · PDF · 이미지 · 캔버스 · 할일을 한 번에 검색하고, 결과를 클릭하면 해당 탭이 열립니다. 입력 필드 / 편집기에 포커스가 있어도 동작합니다.`
        }
      ]
    },
    {
      ...
      rows: [
        {
          label: 'Pane 이동',
          keys: <Combo keys={[isMac() ? '⌃' : 'Ctrl', shift]} trigger="←→↑↓" />,
          description:
            'ctrl + shift 를 누른 상태에서 방향키를 누르면 인접한 pane 으로 active 가 이동합니다. 누르고 있는 동안 layout 미니 그림 오버레이가 표시됩니다.'
        },
        {
          label: '탭 이동',
          keys: <Combo keys={[primary, alt]} trigger="] / [" />,
          description: `${primaryText} + ${altText} 를 누른 상태에서 ] (다음) 또는 [ (이전) 을 누르면 현재 pane 의 탭 사이를 순환합니다. 모디파이어를 떼면 강조된 탭이 활성화됩니다.`
        },
        {
          label: '탭 스냅샷 전환',
          keys: <Combo keys={[primary, shift]} trigger="S" />,
          description: `${primaryText} + shift 를 누른 상태에서 s 를 누르면 워크스페이스의 탭 스냅샷 리스트를 순환합니다. 모디파이어를 떼면 강조된 스냅샷으로 layout 이 복원됩니다.`
        }
      ]
    }
  ]
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">
          현재 지원되는 키보드 단축키 목록입니다. 입력 필드 / 노트 편집기에 포커스가 있을 때는
          비활성화됩니다.
        </p>
      </div>
