...
function blurEvent(contains: boolean): React.FocusEvent<HTMLDivElement> {
  return {
...
describe('handleCellMouseDown', () => {
  ...
  it('7 — Shift+클릭 → anchor 유지, focus 변경', () => {
    ...
    act(() => result.current.handleCellMouseDown(2, 3, mouseEvent({ shiftKey: true })))
    expect(result.current.selection).toEqual({
      anchor: { row: 0, col: 0 },
      focus: { row: 2, col: 3 }
    })
...
describe('selectionRange 계산', () => {
  ...
  it('11 — anchor > focus (역방향) → startRow/startCol = focus, endRow/endCol = anchor', () => {
    ...
    act(() => result.current.handleCellMouseDown(1, 2, mouseEvent({ shiftKey: true })))
    expect(result.current.selectionRange).toEqual({
      startRow: 1,
      endRow: 3,
      startCol: 2,
...
describe('isSingleSelection', () => {
  ...
  it('13 — anchor !== focus → false', () => {
    const { result } = setup()
    act(() => result.current.handleCellMouseDown(0, 0, mouseEvent()))
    act(() => result.current.handleCellMouseDown(1, 1, mouseEvent({ shiftKey: true })))
    expect(result.current.isSingleSelection).toBe(false)
  })
})
