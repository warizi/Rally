...
export function NoteFloatingToolbar({
  ...
  useEffect(() => {
    ...
    const handler = (e: Event): void => {
      ...
      setState(detail)
      // selection 사라지면 color popover 도 자동 닫기.
  ...
  const onToggleItalic = (): void => {
    runCommand(toggleEmphasisCommand.key)
  }
  const onToggleBold = (): void => {
    runCommand(toggleStrongCommand.key)
  }
  const onToggleInlineCode = (): void => {
  ...
  // selection 중심 위쪽에 toolbar 배치. position fixed 라 viewport 기준.
  ...
  const palettteColors = palette[theme]
  ...
  return (
    <div
      ...
      // mousedown 캡처 — editor 의 blur 로 selection 사라지는 것 방지.
