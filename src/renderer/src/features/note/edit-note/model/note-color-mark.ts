 ...
 * ```ts
 * ctx.update(remarkPluginsCtx.key, (prev) => [...prev, colorSpanRemarkPlugin])
 * ctx.update(remarkStringifyOptionsCtx.key, (prev) => ({
 *   ...prev,
 *   handlers: { ...prev.handlers, colorSpan: colorSpanStringifyHandler }
 * }))
 * // 그리고
 * .use(colorMarkSchema)
 * ```
...
/**
 * html 노드의 value 가 color span 시작 토큰인지 확인.
 * 매치되면 색상 값을 반환, 아니면 null.
 */
 ...
 * 매칭되지 않는 span 또는 nested span 은 그대로 둠.
 */
function pairColorSpansInChildren<T extends { children?: unknown[] }>(parent: T): void {
    ...
    if (node.type === 'html') {
      ...
      if (color) {
        ...
        if (closeIdx !== -1) {
          ...
          // 재귀: 내부에 또 다른 paired span 있을 수 있음
  ...
  // mutate parent.children in place
...
export function colorSpanRemarkPlugin() {
  return (tree: Root): void => {
    // paragraph, heading, blockquote, listItem 등 children 을 가진 모든 노드를 방문.
    // mdast 의 phrasingContent 컨테이너 전부 — visit 으로 통합 처리.
    visit(tree, (node) => {
 ...
 *
 * `state.containerPhrasing` 으로 children 을 직렬화한 뒤 raw span 태그로 감싼다.
 */
