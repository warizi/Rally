...
function mockFindById(type: LinkableEntityType, returnValue: any = MOCK_ENTITY) {
  switch (type) {
    case 'todo':
      vi.mocked(todoRepository.findById).mockReturnValue(returnValue)
      break
    case 'schedule':
      vi.mocked(scheduleRepository.findById).mockReturnValue(returnValue)
      break
    case 'note':
      vi.mocked(noteRepository.findById).mockReturnValue(returnValue)
      break
    case 'pdf':
      vi.mocked(pdfFileRepository.findById).mockReturnValue(returnValue)
      break
    case 'csv':
      vi.mocked(csvFileRepository.findById).mockReturnValue(returnValue)
      break
  }
}
```
