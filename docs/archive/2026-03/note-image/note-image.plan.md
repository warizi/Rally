...
interface NoteImageAPI {
  ...
  saveFromBuffer: (workspaceId: string, buffer: ArrayBuffer, ext: string) => Promise<IpcResponse<string>>
  // nodeView 렌더링: .images/ 파일 읽기
  readImage: (workspaceId: string, relativePath: string) => Promise<IpcResponse<{ data: ArrayBuffer }>>
}
```
