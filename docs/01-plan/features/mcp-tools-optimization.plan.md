...
| 10 | `list_todos` | *(신규)* | 투두 목록 조회 (필터: all/active/completed) |
| 11 | `manage_todos` | *(신규)* | 투두 create/update/delete 배치 |
...
// ... 실행 시 타입별로 path 수집
if (noteAffectedPaths.length > 0)
  broadcastChanged('note:changed', wsId, noteAffectedPaths)
if (tableAffectedPaths.length > 0)
  broadcastChanged('csv:changed', wsId, tableAffectedPaths)
```
