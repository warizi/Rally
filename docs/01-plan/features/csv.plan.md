...
function getLeafSiblings(
  ...
  const csvs = csvRepository.findByWorkspaceId(workspaceId)
...
<FolderNameDialog
  ...
  onOpenChange={(open) => { if (!open) setCsvRenameTarget(null) }}
  title="CSV 이름 변경"
...
<DeleteFolderDialog
  ...
  onOpenChange={(open) => { if (!open) setCsvDeleteTarget(null) }}
...
function deleteColumn(index: number): void {
  pushCommand({ type: 'deleteColumn', index, name: headers[index], columnData: data.map(row => row[index]) })
}
...
onMutate: ({ csvId }) => { markAsOwnWrite(csvId) }
