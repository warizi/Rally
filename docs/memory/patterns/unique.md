...
export function createTabId(pathname: string): string {
  return `tab-${pathname.replace(/[^a-zA-Z0-9]/g, '-').replace(/-+/g, '-').replace(/^-|-$/g, '')}`
}
...
export const PANE_ROUTES: PaneRoute[] = [
  ...
  { pattern: '/note-folder/:id', component: NoteFolderPage },
]
```
...
export function createInitialState(): TabState {
  ...
  const mainPane = createPane({ id: 'main', tabIds: [dashboardTab.id], activeTabId: dashboardTab.id })
  return { tabs: { [dashboardTab.id]: dashboardTab }, panes: { [mainPane.id]: mainPane }, layout, activePaneId: 'main' }
}
