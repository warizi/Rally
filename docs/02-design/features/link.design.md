...
```typescript
function normalize(
  typeA: LinkableEntityType, idA: string,
  typeB: LinkableEntityType, idB: string
...
function findEntity(type: LinkableEntityType, id: string) {
  switch (type) {
    ...
    case 'schedule': return scheduleRepository.findById(id)
...
export const entityLinkService = {
  ...
  getLinked(entityType: LinkableEntityType, entityId: string): LinkedEntity[] {
    ...
    const orphanRows: { sourceType: string; sourceId: string; targetType: string; targetId: string }[] = []
    ...
    for (const orphan of orphanRows) {
      entityLinkRepository.unlink(
        orphan.sourceType, orphan.sourceId,
        orphan.targetType, orphan.targetId
      )
    }
...
export function registerEntityLinkHandlers(): void {
  ipcMain.handle(
    'entityLink:link',
    (_: IpcMainInvokeEvent, typeA: LinkableEntityType, idA: string, typeB: LinkableEntityType, idB: string, workspaceId: string): IpcResponse =>
      handle(() => entityLinkService.link(typeA, idA, typeB, idB, workspaceId))
  )

  ipcMain.handle(
    'entityLink:unlink',
    (_: IpcMainInvokeEvent, typeA: LinkableEntityType, idA: string, typeB: LinkableEntityType, idB: string): IpcResponse =>
      handle(() => entityLinkService.unlink(typeA, idA, typeB, idB))
  )
...
export function useLinkedEntities(entityType: LinkableEntityType, entityId: string | undefined) {
  return useQuery({
    ...
    queryFn: async (): Promise<LinkedEntity[]> => {
      const res: IpcResponse<LinkedEntity[]> = await window.api.entityLink.getLinked(entityType, entityId!)
...
// === 아이콘/라벨 매핑 ===
export const ENTITY_TYPE_CONFIG: Record<LinkableEntityType, {
