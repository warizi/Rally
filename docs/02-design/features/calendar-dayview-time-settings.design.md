...
const { data } = useQuery({
  ...
  queryFn: async () => {
    ...
    return {
      startHour: startRes.success && startRes.data ? parseInt(startRes.data, 10) : DEFAULT_START_HOUR,
...
// After
export function getTimeSlots(startHour: number = DEFAULT_START_HOUR, endHour: number = DEFAULT_END_HOUR): TimeSlot[] {
...
// After
export function timeToPosition(date: Date, hourHeight: number, startHour: number = DEFAULT_START_HOUR): number {
