...
- **스킬(Skill)** — 작업 흐름을 `SKILL.md`로 패키징해 Claude·Codex에 배포 (내장 + 커스텀 생성)
...
> **연동 전에 알아두세요**
>
> MCP 연동은 단순한 "기능 켜기"가 아니라 **데이터 접근 권한을 여는 것**입니다.
>
> - 연결된 AI 클라이언트가 해당 워크스페이스의 **모든 노트·할 일·일정·캔버스를 읽고 수정**할 수 있습니다.
> - 대화 과정에서 그 내용이 **해당 서비스 제공사의 서버로 전송**될 수 있으며, 서버는 국외에 있을 수 있습니다.
> - 연결은 설정 화면의 **'제거'** 로 언제든 끊을 수 있습니다.
>
> 일기·건강 기록·계약 정보·타인의 연락처처럼 민감한 내용이 있다면 연결 전에 다른 워크스페이스로 분리하는 것을 권합니다.
> 앱에서도 등록 시 동일한 내용을 안내하고 확인을 받습니다.

### Rally가 하는 외부 통신

Rally는 **사용자 데이터를 수집하거나 자체 서버로 전송하지 않습니다.** 계정도, 서버도, 애널리틱스도 없습니다.
앱이 직접 하는 외부 통신은 다음 두 가지뿐이며, 이때 GitHub에 IP가 전달됩니다.

| 통신                   | 시점                         | 대상            |
| ---------------------- | ---------------------------- | --------------- |
| 업데이트 확인·다운로드 | 앱 시작 시                   | GitHub Releases |
| 임베딩 모델 다운로드   | 검색 기능 최초 사용 시 (1회) | GitHub Releases |

그 외에 노트 내용이 외부로 나가는 경로는 **위의 MCP 연동을 활성화한 경우뿐**입니다.
모든 데이터는 로컬 SQLite와 워크스페이스 폴더의 파일로만 저장됩니다.
...
Rally는 자주 쓰는 AI 작업 흐름을 **스킬**로 패키징해 Claude·Codex에 배포합니다.
설정의 스킬 관리에서 내장 스킬을 켜고 끄거나, 직접 커스텀 스킬을 만들 수 있습니다.

- **내장 스킬 적용** — `rally`, `rally-do`, `rally-plan`, `rally-study` 등 번들 스킬을
  Claude(`~/.claude/skills/`) · Codex(`~/.agents/skills/`)에 토글로 적용/해제
- **커스텀 스킬 생성** — 이름(`rally-` 접두사) · 설명 · 사용할 MCP 도구 · 트리거 키워드 · 본문을
  입력하면 `SKILL.md`가 자동 어셈블되어 배포됩니다 (우측 실시간 미리보기)
- **편집 / 복원** — 본문·도구·트리거를 사용자별로 덮어쓰고, "기본값으로 복원"으로 되돌리기

![커스텀 Skill 등록](assets/skill-set.png)

### 예시 — 노트로 학습하기 (`rally-study`)

노트 내용을 요약 → 세부 학습 → 퀴즈 → 보충으로 반복하는 학습 스킬. "학습하자"라고 말하면
Rally의 노트를 읽어 강의·퀴즈 세션을 진행합니다.

```markdown
---
name: rally-study
description: |
  rally-study [폴더명 또는 노트명]
  노트 내용을 기반으로 사용자에게 학습하도록 내용 정리, 세부내용 학습, 퀴즈 등을 제공한다.
  Triggers: 학습하자, 퀴즈내줘, 공부하자.
---

# rally-study

## Phase 1 노트의 내용을 요약 설명한다.

## Phase 2 각 부분별로 세부 설명하며 사용자와 상호작용한다.

## Phase 3 노트 내용을 기반으로 퀴즈를 낸다.

## Phase 4 퀴즈 기반으로 부족한 내용을 보충 설명한다.

## Phase 5 멈춤 요청 전까지 Phase 2~5를 반복한다.
...
이처럼 워크플로우를 스킬로 정의해두면, 같은 작업을 매번 설명할 필요 없이 트리거 한 마디로 재현할 수 있습니다.
