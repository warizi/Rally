...
할일·노트·캔버스·캘린더를 하나로 통합하고, MCP를 통해 Claude가 앱을 직접 읽고 제어합니다.
...
- Claude Desktop / Claude Code가 Rally의 할일·노트를 직접 읽고 씁니다
...
- 앱 내 HTTP MCP 서버로 Claude Desktop · Claude Code 연동
...
설정에서 **Claude Desktop / Claude Code 연동**을 켜면, Rally가 각 클라이언트 설정 파일에
MCP 서버(`rally`)를 자동 등록합니다.
...
등록 후 Claude를 재시작하면 17개 MCP 도구가 노출됩니다.
